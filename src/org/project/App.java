...
Если пустое, тогда предлагаем ввести id работника. */
    ...
    private static void clearWorker() {
        if ("yes".equals(data)) {
            org.clear();
            System.out.println("Все работники уволены и вы несете убытки. Быстрей нанимайте новых сотрудников.\nДля продолжения жмите enter\n");
            path = "";
        } else if ("no".equals(data)) {
            System.out.println("Вы сделали все правильно. Работники в вашу честь устроили пати. Ваше предприятие приносит деньги и вы счастливый человек.");
            path = "";
        } else {
            System.out.println("Вы точно хотите выгнать всех работников предприятия?\nВам придется выплатить деньги за выходное пособие.\nВы можете понести большие убытки.\n\nyes - Да, пусть катятся куда подальше и выдать им деньги.\nno - нет, я работников ценю и увольнять не хочу.");
            path = "/clear";
        }
    }
